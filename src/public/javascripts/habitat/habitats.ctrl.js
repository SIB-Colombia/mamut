...
.controller('HabitatsCtrl', ['$scope','$http','ReferenceFactory', 'AncillaryDataFactory', function($scope,$http,ReferenceFactory,AncillaryDataFactory) {
	...
	$scope.addHabitat = function(){
		var req_1 = {
			method: 'POST',
			url: 'http://192.168.205.191:8080/fichas/'+$scope.formData._id+'/habitats/',
			headers: {
			  'Content-Type': 'application/JSON'
			},
			data: { "id_user" : "01",
				"habitats":$scope.formData.habitats
	...
	$scope.removeHabitatAtomized= function(list,habitatAtomized){
		$scope.habitatsFactoryLocal.delete(list,habitatAtomized);	
	};
            	...
            	if(ancillaryData.license === item.nombre){
					...
					if(item.nombre==='Otra' && !checked_almost_one){
					 	if(document.getElementById('ancillaryData.license') === null){
				            ...
				            document.getElementById("ManualLicenseHabitat").appendChild(input);
						}else{
							var license = document.getElementById("ancillaryData.license");
