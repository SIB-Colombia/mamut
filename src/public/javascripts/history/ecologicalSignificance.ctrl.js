...
.controller('EcologicalSignificanceCtrl', ['$scope','$http','ReferenceFactory', 'AncillaryDataFactory', function($scope,$http,ReferenceFactory,AncillaryDataFactory) {
	...
	$scope.addEcologicalSignificance = function(){
		var req_1 = {
			method: 'POST',
			url: 'http://apichigui-env.us-east-1.elasticbeanstalk.com/fichas/'+$scope.formData._id+'/ecological_significance/',
			headers: {
			  'Content-Type': 'application/JSON'
			},
			data: { "id_user" : $scope.useremail,
				"ecologicalSignificance":$scope.formData.ecologicalSignificance
	...
	$scope.removeEcologicalSignificanceAtomized= function(list,ecologicalSignificanceAtomized){
		$scope.ecologicalSignificanceFactoryLocal.delete(list,ecologicalSignificanceAtomized);	
	};
            	...
            	if(ancillaryData.license === item.nombre){
					...
					if(item.nombre==='Otra' && !checked_almost_one){
					 	if(document.getElementById('ancillaryData.license') === null){
				            ...
				            document.getElementById("ManualLicenseEcological").appendChild(input);
						}else{
							var license = document.getElementById("ancillaryData.license");
