...
.controller('TerritoryCtrl', ['$scope','$http','ReferenceFactory', 'AncillaryDataFactory', function($scope,$http,ReferenceFactory,AncillaryDataFactory) {
	...
	$scope.addTerritory = function(){
		var req_1 = {
			method: 'POST',
			url: 'http://192.168.205.17:8080/fichas/'+$scope.formData._id+'/territory/',
			headers: {
			  'Content-Type': 'application/JSON'
			},
			data: { "id_user" : "01",
				"territory":$scope.formData.territory
            	...
            	if(ancillaryData.license === item.nombre){
					...
					if(item.nombre==='Otra' && !checked_almost_one){
					 	if(document.getElementById('ancillaryData.license') === null){
				            ...
				            document.getElementById("ManualLicenseTerritory").appendChild(input);
						}else{
							var license = document.getElementById("ancillaryData.license");
